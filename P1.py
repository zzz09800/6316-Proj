...
df = pd.read_csv('./data/Data2011.csv', header=0)
...
def driverGenderNormalization(str):
    str = str.lower()
    if "male" in str:
        return "Male"
    if "female" in str:
...
selected_df['DRIVERGEN'] = df['DRIVERGEN'].apply(driverGenderNormalization)

selected_df = selected_df.apply(lambda x: pd.factorize(x)[0])

# print selected_df

# use 3/4 of the dataset as training set.
selected_df['is_train'] = np.random.uniform(0, 1, len(df)) <= 0.90

train = selected_df[selected_df['is_train']==True]
test = selected_df[selected_df['is_train']==False]
features = selected_df.columns[0:5]

# print features
# print train
# print test

forest = RFC(n_jobs=8, n_estimators=60)
